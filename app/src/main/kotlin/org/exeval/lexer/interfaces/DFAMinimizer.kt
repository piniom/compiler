...
class DFAmin<S> : DFAMinimizer<S> {
    ...
    companion object{
        fun test(){
            /*
            quick test, useful since it detects infinite loops
            */
            var start = "A"
            ...
            var miner = DFAmin<String>()
            var base = minDFA<String>(start, acs, t)
            var min = miner.minimize(base)
            println(miner.getStates(min))
        }
    }
}

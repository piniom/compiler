...
class DFAParserImpl<S> : DFAParser<S> {
	...
	private class ParsedDFA : DFA<Int> {
		...
		override fun isDead(state: Int): Boolean = !isAccepting(state) && transitions(state).all { it.value == state }

		override fun isAccepting(state: Int): Boolean = acceptStates.contains(state)

		override fun transitions(state: Int): Map<Char, Int> = tmap[state]!!.toMap()
	}
	...
	override fun parse(nfa: NFA<S>): DFA<*> {
		...
		return ParsedDFA(
			startConfigNumber,
			acceptingConfigsNumbers,
			configsTransitions,
		)
	}

	private fun containsAcceptingState(
		nfa: NFA<S>,
		states: Set<S>,
	): Boolean = states.any { s -> nfa.acceptingState == s }

	private fun getConfigNumber(config: Set<S>): Int =
		(
			configsNumbers[config] ?: run {
				nextConfigNumber++
	...
	private fun getAllETransitions(
		nfa: NFA<S>,
		state: S,
	): Set<S> {

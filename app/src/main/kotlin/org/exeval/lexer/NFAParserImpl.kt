...
class NFAParserImpl(fabric: (Int) -> Any) : NFAParser {
	val stateFabric: (Int) -> Any = fabric
	...
	inner class NFAImpl<Any> : NFA<Any> {
		...
		override fun transitions(state: Any): Map<Char, Any> = trans[state]?.toMap() ?: emptyMap()

		override fun eTransitions(state: Any): Set<Any> = eTrans[state]?.toSet() ?: emptySet()
		...
		private fun computeDirectEpsilonClosure(
			state: Any,
			states: Set<Any>,
		): MutableSet<Any> {
	...
	fun parseInner(
		...
		return when (regex) {
			...
			is Regex.Union -> {
				val nfas =
					regex.expressions.map { expression ->
	...
	override fun parse(regex: Regex): NFA<Any> = parseInner(regex, 0)
}

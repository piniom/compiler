...
class DFAmin<S> : DFAMinimizer<S> {
	...
	private class minDFA<S> : DFA<S> {
		...
		override fun isAccepting(state: S): Boolean = acceptStates.contains(state)

		override fun transitions(state: S): Map<Char, S> = tmap[state]!!.toMap()
	}
	...
	override fun minimize(dfa: DFA<S>): DFA<S> {
		...
		while (true) {
			fun setOf(s: S): MutableSet<S> = pastSets.find { it.contains(s) }!!
			// build new partition

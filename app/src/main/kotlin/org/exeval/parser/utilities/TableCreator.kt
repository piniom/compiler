...
class TableCreator<S>(
	private val analyzedGrammar: AnalyzedGrammar<S>,
) {
	private val productionMap: Map<S, Set<Production<S>>> =
		analyzedGrammar.grammar.productions
			.groupBy { prod ->
				prod.left
	...
	private fun getTableGoto(
		allStates: Set<Parser.State<S>>,
		oldGoto: Map<Pair<S, Parser.State<S>>, Parser.State<S>>,
				...
				when {
					!isAtTheEndOfProduction -> {
						...
						if (oldVal != null && oldVal != newVal && oldVal !is Action.Reduce) {
							throw TableCreationError(
								"There is conflict in an action table. Grammar is probably ambiguous. Entry actions[$placeholderSymbol, $curCC] have two values: $oldVal and $newVal",
							)
						}
					...
					isAtTheEndOfProduction && isLookaheadTerminal -> {
						...
						if (oldVal != null && oldVal is Action.Shift<*, *>) {
							...
							throw TableCreationError(
								"There is conflict in an action table. Grammar is probably ambiguous. Entry actions[${item.lookahead}, $curCC] have two values: $oldVal and $newVal",
							)
						} else {
	...
	private fun getStartingState(): Parser.State<S> {
		val allStartingProductions =
			productionMap[analyzedGrammar.grammar.startSymbol]
				?: throw IllegalStateException("No productions for start symbol")

		val startingState =
			allStartingProductions
				.map { prod -> Parser.LR1Item<S>(prod, 0, analyzedGrammar.grammar.endOfParse) }
				.toSet()
				.let { Parser.State(it) }
				.getClosure()
	...
	private fun getAllStatesAndOldGoto(
		startingState: Parser.State<S>,
	): Pair<Set<Parser.State<S>>, Map<Pair<S, Parser.State<S>>, Parser.State<S>>> {
	...
	private fun List<S>.getFIRST(): Set<S> {
		...
		for ((i, s) in this.withIndex()) {
			val firstForS =
				if (s in productionMap) {
					analyzedGrammar.firstProduct[s]
	...
	private fun Parser.State<S>.getClosure(): Parser.State<S> {
		...
		while (queue.isNotEmpty()) {
			...
			val lookaheadString =
				item.production.right.subList(
					item.placeholder + 1,
					item.production.right.size,

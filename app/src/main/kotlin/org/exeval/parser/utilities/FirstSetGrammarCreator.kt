...
class FirstSetGrammarCreator {
	fun <S> createFirstSet(
		grammar: Grammar<S>,
		nullable: Set<S>,
	): Map<S, Set<S>> =
		transitiveClosure(
			addPossibleStates(
				crateMapWithOnlyKeySymbol(grammar),
				grammar,
				nullable,
			),
	...
	private fun <S> crateMapWithOnlyKeySymbol(grammar: Grammar<S>): Map<S, MutableSet<S>> =
		grammar.productions
			.map {
				it.left
			}.associateWith { mutableSetOf(it) }

	private fun <S> addPossibleStates(
		first: Map<S, MutableSet<S>>,
		grammar: Grammar<S>,
		nullable: Set<S>,
	): Map<S, MutableSet<S>> {
	...
	private fun <S> transitiveClosure(first: Map<S, MutableSet<S>>): Map<S, MutableSet<S>> {
		repeat(first.size) {
			first.forEach { (_, set) ->
				set.addAll(
					set.fold(mutableSetOf()) { newSet, symbol ->

...
class NameResolutionGenerator(
	private val astInfo: AstInfo,
) {
	class consts {
		companion object {
	...
	data class LoopStackData(
		var closestLoop: Loop? = null,
		val loopMap: MutableMap<String, Loop> = mutableMapOf(),
	)
	...
	private fun assignArguments(
		call: FunctionCall,
		decl: AnyFunctionDeclaration,
	) {
		var hasNamed = false
		var positionalIdx = 0
	...
	private fun hasSameLoopIdentifierInScope(name: String): Boolean = loopStack.any { it.loopMap.containsKey(name) }
	...
	private fun processNamedBreak(
		breakNode: Break,
		name: String,
	) {
		getLoopData().loopMap[name]?.let {
			breakToLoop[breakNode] = it
		}
	...
	private fun getClosestLoop(): Loop? = getLoopData().closestLoop

	private fun getLoopData(): LoopStackData = loopStack.last()

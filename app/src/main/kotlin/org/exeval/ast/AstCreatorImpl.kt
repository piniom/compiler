...
class AstCreatorImpl : AstCreator<GrammarSymbol> {
	...
	override fun create(
		parseTree: ParseTree<GrammarSymbol>,
		input: Input,
	): AstInfo = AstInfo(createAux(parseTree, input), locationsMap)

	private fun createAux(
		node: ParseTree<GrammarSymbol>,
		input: Input,
	): ASTNode {
		...
		if (symbol === ProgramSymbol) {
			...
			val type =
				getType(children[typeIndex], input) ?: throw IllegalStateException("AllocationSymmbol $locationRange")
			...
			astNode =
				processArrayAccess(children, input) ?: throw IllegalStateException("ArrayAcessSymbol $locationRange")
		} else {
	...
	private fun processArrayAccess(
		children: List<ParseTree<GrammarSymbol>>,
		input: Input,
	): Expr? {
		fun processNestedArray(
			arrayExpr: Expr,
			indexSymbol: ParseTree<GrammarSymbol>,
			input: Input,
		): Expr? {
			val nextIndexExpr = 3
	...
	private fun parseFunctionCallArguments(
		node: ParseTree<GrammarSymbol>,
		input: Input,
	): List<Argument> {
		...
		for (child in children) {
			...
			when {
				childSymbol === ExpressionSymbol && isVariableAssignment(child) -> {
					...
					arguments.add(
						NamedArgument(
							getNameOfVariableToAssign(variableAssignment.variable),
	...
	private fun parseExpressionChain(
		node: ParseTree<GrammarSymbol>,
		input: Input,
	): List<Expr> {
	...
	private fun findSubTrees(
		head: ParseTree<GrammarSymbol>,
		symbol: GrammarSymbol,
	): List<ParseTree<GrammarSymbol>> {

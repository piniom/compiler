...
class CFGMaker(
	...
	private fun walkBinaryOperation(
		bin: BinaryOperation,
		then: CFGNode,
	): WalkResult {
	...
	private fun walkConditional(
		...
		saveElse.trees =
			if (thenBranch.tree != null) { // TODO: watch out - this is null when branch only contains break - check if this is intended
	...
	private fun walkLogicalExpressionShortCircuit(
		expr: Expr,
		then: CFGNode,
	): WalkResult {
		val reg = newVirtualRegister()
	...
	private fun walkNopeConditional(
		conditional: Conditional,
		then: CFGNode,
	): WalkResult {
	...
	private fun walkUnaryOperation(
	...
	private fun walkVariableDeclarationBase(
		variableDeclaration: VariableDeclarationBase,
		then: CFGNode,
	): WalkResult {
		if (variableDeclaration.initializer == null || typeMap[variableDeclaration.initializer!!]!!.isNope()) {
			return WalkResult(
				then,
				null,
			)
	...
	private fun walkVariableReference(
	...
	private fun newVirtualRegister(): AssignableTree = RegisterTree(VirtualRegister())

	private fun convertBinOp(operation: BinaryOperator): BinaryOpType =
		when (operation) {
			BinaryOperator.PLUS -> BinaryOpType.ADD
			BinaryOperator.MINUS -> BinaryOpType.SUBTRACT
			BinaryOperator.MULTIPLY -> BinaryOpType.MULTIPLY
			BinaryOperator.DIVIDE -> BinaryOpType.DIVIDE
			BinaryOperator.AND -> BinaryOpType.AND
			BinaryOperator.OR -> BinaryOpType.OR
			BinaryOperator.EQ -> BinaryOpType.EQUAL
			BinaryOperator.GT -> BinaryOpType.GREATER
			BinaryOperator.GTE -> BinaryOpType.GREATER_EQUAL
			BinaryOperator.LT -> BinaryOpType.LESS
			BinaryOperator.LTE -> BinaryOpType.LESS_EQUAL

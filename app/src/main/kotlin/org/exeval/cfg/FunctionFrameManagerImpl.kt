...
class FunctionFrameManagerImpl(
	...
	val label: Label
	private val calleSaveRegisters =
		listOf(
			PhysicalRegister.R9,
	...
	override fun generate_var_access(
		x: AnyVariable,
		functionFrameOffset: Tree,
	): AssignableTree {
	...
	private fun backupRegisters(): List<Tree> {
		...
		return calleSaveRegisters
			.map {
				pushToStack(RegisterTree(it))
			}.flatten()
	}

	private fun restoreRegisters(): List<Tree> = calleSaveRegisters.map { popFromStack(RegisterTree(it)) }.flatten()
	...
	private fun pushToStack(tree: Tree): List<Tree> = listOf(StackPushTree(tree))

	private fun popFromStack(toAssign: AssignableTree): List<Tree> = listOf(StackPopTree(toAssign))

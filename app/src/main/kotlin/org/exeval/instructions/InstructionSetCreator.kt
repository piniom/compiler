...
class InstructionSetCreator {
		...
		(
			createAssignmentPatterns() +
				createSafeSimple2ArgPatterns(
					BinaryAddTreeKind,
					{ par1: OperandArgumentType, par2: OperandArgumentType ->
						AddInstruction(
							par1 as AssignableDest,
							par2,
						)
					},
				) +
				...
				createSimpleComparisonPatterns(
					BinaryGreaterEqualTreeKind,
					OperationAsm.CMOVGE,
					OperationAsm.JGE,
				) +
	...
	private fun createSimpleBoolOperationPatterns(
		...
		return listOf(
			TemplatePattern(rootOperation, InstructionKind.VALUE, 1) { dest, inputs, _ ->
				...
				convertBooleanTo0Or1(reg1, inputs[0]) +
					listOf(
	...
	private fun createEmptyExecPattern(rootType: TreeKind): TemplatePattern =
		TemplatePattern(rootType, InstructionKind.EXEC, 1) { _, _, _ -> listOf() }

	private class LabelDeclarationInstruction(
		private val label: Label,
	) : Instruction {
		override fun toAsm(mapping: Map<Register, PhysicalRegister>): String = label.name + ":"

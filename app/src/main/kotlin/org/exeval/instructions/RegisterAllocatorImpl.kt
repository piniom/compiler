...
class RegisterAllocatorImpl : RegisterAllocator {
	override fun allocate(
		...
		val coalescence: RegisterGraph =
			CoalescenceGraphCreator().createCoalescenceGraph(graph, copyGraph, domain, range.size)
		...
		for (vertex in order) {
			...
			if (physicalRegister !== null) {
				...
				val availableRegisters =
					range subtract graph[vertex]!!.filter { colored.contains(it) }.map { mapping[it]!! }

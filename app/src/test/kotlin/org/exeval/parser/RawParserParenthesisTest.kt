...
class RawParserParenthesisTest {
	...
	private val grammar =
		Grammar(
			startSymbol = ParSym.LIST,
			endOfParse = ParSym.END,
			productions =
			listOf(
	...
	private val actions: Map<Pair<ParSym, Int>, Action<ParSym, Int>> =
		mapOf(
	...
	private val goto: Map<Pair<ParSym, Int>, Int> =
		mapOf(
	...
	private fun stringToLeaves(str: String): List<ParseTree.Leaf<ParSym>> {
		val body =
			str.filter { c -> c == '(' || c == ')' }.mapIndexed { i, char ->
				val loc = Loc(i)
				val res: ParseTree.Leaf<ParSym> =
					when (char) {

...
class RegisterAllocation {
	...
	fun sanity(
		...
		// no conflict
		assert(
			data.livenessResult.interference.all {
				val key = it.key
				it.value.all {
	...
	fun simulate(
		instructionList: List<instructions.base>,
		a: AllocationResult,
	) {
		assert(a.spills.isEmpty())
		val tInstructionsList: List<instructions.base> =
			instructionList.map {
				when (it) {

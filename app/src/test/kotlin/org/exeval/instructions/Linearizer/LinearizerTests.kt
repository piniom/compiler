...
class LinearizerTest {
	...
	fun `test simple loop block`() {
		...
		assertEquals(
			1,
			result[0].instructions.size,
			"Expected one instruction in the first basic block. (There is no JMP due to optimization)",
		)
	...
	fun `test complex loop block`() {
		...
		assertEquals(
			1,
			result[1].instructions.size,
			"Expected one instruction in the second basic block. (There is no JMP due to optimization).",
		)

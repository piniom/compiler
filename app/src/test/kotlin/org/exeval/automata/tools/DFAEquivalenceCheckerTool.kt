...
class DFAEquivalenceCheckerTool : DFAEquivalenceChecker {
	override fun <S, T> areEquivalent(
		dfa1: DFA<S>,
		dfa2: DFA<T>,
	): Boolean {
	...
	// /</summary>
	private fun <S, T> checkIfShouldCountinue(
		dfa1: DFA<S>,
		dfa2: DFA<T>,
		entry: Map.Entry<Char, S>,
		secondMap: Map<Char, T>,
	): Boolean? {
	...
	private fun <S, T> setState(
		map: MutableMap<S, MutableMap<T, Boolean>>,
		state1: S,
		state2: T,
		value: Boolean,
	) {
